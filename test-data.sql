-- Sample test data for exam system (matching TypeORM schema)
...
-- Insert a question pool (NO cognitive_level column)
...
-- Insert sample MCQ questions (using correct column names from TypeORM)
INSERT INTO questions (id, pool_id, question_type, content, cognitive_level, data) VALUES 
('33333333-3333-3333-3333-333333333333', '22222222-2222-2222-2222-222222222222', 'MCQ', 'HTML là viết tắt của?', 1,
...
('44444444-4444-4444-4444-444444444444', '22222222-2222-2222-2222-222222222222', 'MCQ', 'CSS được sử dụng để làm gì?', 1,
'{"options": [{"id": "b1", "text": "Định dạng giao diện trang web"}, {"id": "b2", "text": "Tạo cơ sở dữ liệu"}, {"id": "b3", "text": "Xử lý logic ứng dụng"}, {"id": "b4", "text": "Lưu trữ dữ liệu"}], "correct_option_id": "b1"}')
ON CONFLICT (id) DO NOTHING;

-- Insert an exam matrix (using correct column names)
INSERT INTO exam_matrices (id, subject_id, name, settings) VALUES 
('55555555-5555-5555-5555-555555555555', '11111111-1111-1111-1111-111111111111', 'Ma trận đề thi Tin học THPT 2024',
'{"mcq_rules": [{"pool_id": "22222222-2222-2222-2222-222222222222", "count": 2, "cognitive_level": 1}], "group_rules": []}')
ON CONFLICT (id) DO NOTHING;

-- Insert an exam session (using correct column names)
INSERT INTO exam_sessions (id, name, matrix_id, start_time, end_time, duration_minutes, status) VALUES 
('66666666-6666-6666-6666-666666666666', 'Kỳ thi Tin học THPT 2024', '55555555-5555-5555-5555-555555555555', 
...
('77777777-7777-7777-7777-777777777777', 'SV001', 'Nguyễn Văn A', '2005-01-01', '12A1')
